...
const FinishMeterSetup = ({navigation, route}) => {
    ...
    if(wifiPassword === ''){
      ...
      if(wifiPassword === '1234' || wifiPassword === '0000'){
        ...
        else
        {
          ...
          const payload = {
            name: route.params.SmartMeterId,
            type: route.params.SmartMeterType,
            status : "true"
          }

          await postData('smart-utility-iot/hub/' + route.params.IoTHubId + '/devices', true, payload).then( async (response) => {
              if (response !== undefined) {
                
                if(response.id !== undefined){

                  if (response.id != undefined && response.id.length > 0) {
                    Alert.alert("Message", "Your Smart Meter is successfully setup and ready to work with your designated Central IoT Hub after a reboot");
                    navigation.navigate('Dashboard');
                  } else {
                    Alert.alert("Error", "Unable to add your Smart Meter device.");
                  }
  ...
  return (
    ...
    <Text style={styles.subText}>Connecting your new SMart Meter to your wifi network</Text>

      <View style={styles.mainView}>
      <Text style={styles.subText}>SSID : {route.params.IoTHubWifiSSID} </Text>
...
export default FinishMeterSetup;
